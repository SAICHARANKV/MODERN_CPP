...
/*
    ...
    Hyper Threading - A single thread can do 2 process.When there is a delay in
   one process for a time processor or OS could take different process (context
   switching) and run untill first delay completes.
...
float AverageofNValues(std::future<int> &ft) {
  ...
  float sum = 0.0f;
  int count = 0;
  // Make Average Thread Wait(go on hold)till ft.get() returns
  // a signal IF THERE IS ANY DATA IN FUTURE,WE CAN SAVE IT IN N
  int N = ft.get();
  if (data.size() < N || N <= 0) {
...
int main() {
  ...
  std::future<float> result =
      std::async(std::launch::async, AverageofNValues, std::ref(ft));

  // ft has to be passed by reference as it makes a duplication of ft object and
  // pr cannot communicate with ft as it is duplicated.
