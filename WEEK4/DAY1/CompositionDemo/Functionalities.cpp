...
std::optional<Container> FindMatchingAccounts(const Container &data)
{
    ...
    std::size_t newSize = std::distance(result.begin(), itr); // used to subtract two same objects
...
/*
If default constructor is deleted or segmentation fault means that data intitalization of memory is
not happening as in line 129 data.size() solves error

If there is direct copy it is fine.If it has to return a container based on a condition data resizing is
must

If you want to copy whole object normal copy process can be done. If Condition Based copy
